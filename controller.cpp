...
static Controller* m_instance = nullptr;
...
void Controller::setMainView(QObject* mainView)
{
    m_mainView = mainView;
}
...
void Controller::addNumber(QString expression)
{
    string exp = expression.toStdString();
    exp.pop_back();
    qDebug() << exp.c_str();
    float result = tinh(exp);
    qDebug() << "Hai" <<result;
    m_mainView->setProperty("result", result);
}
